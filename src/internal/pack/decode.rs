...
impl Pack {
        ...
        C: FnOnce(ObjectHash) + Send + 'static,
    {
        ...
        while i < self.number {
            ...
            // 3 parts: Waitlist + ThreadPool + Caches. Pause when queued work or memory
            // pressure crosses the decode backpressure policy.
...
mod tests {
    ...
    #[test]
    fn test_cache_memory_limit_uses_configured_ratio() {
        assert_eq!(super::cache_memory_limit(100), 80);
        assert_eq!(
            super::cache_memory_limit(usize::MAX),
            ((usize::MAX as u128) * 4 / 5) as usize
        );
    }

    #[test]
    fn test_should_pause_decode_when_queue_exceeds_limit() {
        assert!(!super::should_pause_decode(
            super::DECODE_QUEUE_BACKPRESSURE_LIMIT,
            usize::MAX,
            None
        ));
        assert!(super::should_pause_decode(
            super::DECODE_QUEUE_BACKPRESSURE_LIMIT + 1,
            0,
            None
        ));
    }
    ...
    #[test]
    fn test_should_pause_decode_when_memory_exceeds_limit() {
        assert!(!super::should_pause_decode(0, 1024, Some(1024)));
        assert!(super::should_pause_decode(0, 1025, Some(1024)));
    }

    #[test]
    fn test_should_not_pause_decode_without_memory_limit() {
        assert!(!super::should_pause_decode(0, usize::MAX, None));
    }
